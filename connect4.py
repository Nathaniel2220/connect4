...
try:
    ...
    class App(tk.Tk):
        def __init__(self, players = None, player_colors = None, num_columns = 7, num_rows = 6):
            ...
            self.top_banner.grid(column=1, row=1, columnspan=7)
            ...
            self.canvas = tk.Canvas(width = 700, height=600, bg=BACKGROUND_COLOR, highlightthickness=0)
            self.canvas.grid(column=1, row=3, columnspan=7)
        ...
        def _finish_turn(self, dropped_disc):
            ...
            win_location = self._find_win(column)
            if win_location:
                self._declare_victory(self.current_player, win_location)
            elif not self._is_valid_move():
                self.top_banner.config(text="Tie Game", fg="white")
            else: self._swap_player()
                
        # check for victory in a given column (or all)--returns tuples on win, or None
        def _find_win(self, column = None):

            # if no column explicitly given, do them all
            if column == None:
                for c in range(len(self.rack)):
                    win = self._find_win(c)
                    if win: return win
                return None

            num_cols = len(self.rack)
            num_rows = len(self.rack[0])
            
            # figure out where the disc was dropped
            row = num_rows - 1
            while row > -1 and self.rack[column][row] == 0: row -= 1
            if row == -1: return None            

            player = self.rack[column][row]

            # check for vertical win
            if row >= 3:
                subrack = self.rack[column]
            ...
            c = d = column
            while c > 0 and self.rack[c-1][row] == player: c -= 1
            while d < (num_cols-1) and self.rack[d+1][row] == player: d += 1
            if (d-c) >= 3: return ((c, row), (d, row))

            # check for forward-diagonal win
            c = d = column
            r = s = row
            while c > 0 and r > 0 and self.rack[c-1][r-1] == player: c -= 1; r -= 1
            while d < (num_cols-1) and s < (num_rows-1) and self.rack[d+1][s+1] == player: d += 1; s+=1
            if (d-c) >= 3: return ((c, r), (d, s))
            
            # check for backward-diagonal win
            c = d = column
            r = s = row
            while c > 0 and r < (num_rows-1) and self.rack[c-1][r+1] == player: c -= 1; r += 1
            while d < (num_cols-1) and s > 0 and self.rack[d+1][s-1] == player: d += 1; s-=1
            if (d-c) >= 3: return ((c, r), (d, s))
        ...
        # return True if at least 1 column is still open
        def _is_valid_move(self):
            for c in range(len(self.rack)):
                if self.rack[c][-1] == 0: return True
            return False
        ...
        def _make_rack_image():
            ...
            return ImageTk.PhotoImage(im.resize((SQUARE_SIZE, SQUARE_SIZE), resample=Image.LANCZOS))
        ...
        def _make_icon(color1, color2):
            ...
            return ImageTk.PhotoImage(im.resize((64, 64), resample=Image.LANCZOS))
